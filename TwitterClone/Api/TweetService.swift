...
struct TweetService {
    ...
    func uploadTweet(caption: String, type: UploadTweetConfiguration, completion: @escaping(DatabaseCompletion)){
        ...
        case .reply(let tweet):
            ...
            REF_TWEEET_REPLIES.child(tweet.tweetID).childByAutoId().updateChildValues(values){(err, ref) in
    ...
    func fetchReplies(forUser user: User, completion: @escaping([Tweet]) -> Void){
        ...
        REF_USER_REPLIES.child(user.uid).observe(.childAdded) {snapshot in
            ...
            REF_TWEEET_REPLIES.child(tweetKey).child(replyKey).observeSingleEvent(of: .value) { (snapshot) in
                guard let dictionary = snapshot.value as? [String: Any] else { return }
                guard let uid = dictionary["uid"] as? String else { return }
    ...
    func fetchReplies(forTweet tweet: Tweet, completion: @escaping([Tweet]) -> Void){
        var tweets = [Tweet]()
        REF_TWEEET_REPLIES.child(tweet.tweetID).observe(.childAdded) {snapshot in
